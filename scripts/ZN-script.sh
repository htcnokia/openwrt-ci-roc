...
package_enabled() {
  ...
  for package_name in "$@"; do
    if config_symbol_enabled "CONFIG_PACKAGE_$package_name" ||
      target_device_package_enabled "$package_name"; then
      return 0
    fi
  done
...
record_git_revision() {
  ...
  grep -Fqx -- "$revision" "$THIRD_PARTY_SOURCES_FILE" ||
...
if [ ! -f "$DEVICE_CONFIG_FILE" ]; then
  echo "Error: ZN-M2 device configuration was not found: $DEVICE_CONFIG_FILE" >&2
  exit 1
fi

if ! grep -q '^CONFIG_TARGET_qualcommax=y$' "$DEVICE_CONFIG_FILE" ||
  ! grep -q '^CONFIG_TARGET_qualcommax_ipq60xx=y$' "$DEVICE_CONFIG_FILE" ||
  ! grep -q '^CONFIG_TARGET_DEVICE_qualcommax_ipq60xx_DEVICE_zn_m2=y$' "$DEVICE_CONFIG_FILE"; then
  echo "Error: the device configuration does not select ZN-M2 on qualcommax/ipq60xx" >&2
  exit 1
fi
...
if [ -f "$luci_system_js" ]; then
  grep -Fq "$firmware_version_anchor" "$luci_system_js" || {
    echo "Error: LuCI firmware version anchor was not found in $luci_system_js" >&2
    exit 1
  }
...
cat "${CONFIG_FILES[@]}" > .config

grep -vE '^CONFIG_TARGET_DEVICE_qualcommax_ipq60xx_DEVICE_' .config > .config.zn-m2
printf '%s\n' 'CONFIG_TARGET_qualcommax=y' >> .config.zn-m2
printf '%s\n' 'CONFIG_TARGET_qualcommax_ipq60xx=y' >> .config.zn-m2
printf '%s\n' 'CONFIG_TARGET_DEVICE_qualcommax_ipq60xx_DEVICE_zn_m2=y' >> .config.zn-m2
mv .config.zn-m2 .config

make defconfig

enabled_devices="$(grep -E '^CONFIG_TARGET_DEVICE_qualcommax_ipq60xx_DEVICE_.*=y$' .config || true)"
device_count="$(printf '%s\n' "$enabled_devices" | sed '/^$/d' | wc -l)"

if [ "$device_count" -ne 1 ] ||
  ! printf '%s\n' "$enabled_devices" |
    grep -qx 'CONFIG_TARGET_DEVICE_qualcommax_ipq60xx_DEVICE_zn_m2=y'; then
  echo "Error: target device validation failed" >&2
  printf '%s\n' "$enabled_devices" >&2
  exit 1
fi
